...
6. Define a deliberate candidate surface in `project.mutablePaths`: one file for simple tuning, several files for coupled model/config work, or a dedicated candidate directory. For open-ended research, expose a complete `candidate/` directory and consider `agent.analysis` so the implementer can run evidence-producing EDA and diagnostic scripts before editing it. Put the evaluator and split logic in `protectedPaths`; additionally put held-out labels, private targets, and hidden scoring logic in `hiddenPaths`.
