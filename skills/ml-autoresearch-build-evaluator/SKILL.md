...
- When `runtimeDependencies` is enabled, import candidate-scoped Python or Bun packages normally. The harness mounts the broker-owned locked overlay into the evaluator and sets `PYTHONPATH`/`NODE_PATH`; do not run `pip`, `bun install`, or any network installer from evaluation. Analysis-scoped packages are intentionally absent. Treat a missing import as an evaluation failure, not as permission to install dynamically.
